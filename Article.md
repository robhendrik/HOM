...
*What happens when we look at reflection from a window one photon at a time? Quantum interference, indistinguishability, and boson symmetry appear in an everyday piece of glass.*
![Feature](Feature_image_mtplb.png)
...
But we should also consider the phase. Imagine two beamsplitters that first split a beam and then combine it again. It cannot be that we end up with two output beams, each with the same intensity as the original input beam. That would create energy from nothing.

A beamsplitter therefore introduces a relative phase between reflection and transmission — but not the same phase everywhere. Reflecting off the front of the glass reflective surface flips the phase by 180°. Reflecting off the back does not. Transmission, in either direction, has no phase flip at all.
...
A beamsplitter therefore introduces a relative phase between reflection and transmission. For a 50/50 beamsplitter, one of these amplitudes picks up a 180° phase shift. This ensures that when we combine multiple beamsplitters, the waves can interfere, but the total energy is always conserved.

## What Does A Single Photon Do At A Window?

What does this mean for a single photon?

The wavefunction follows essentially the same logic as the classical wave. The probability of finding the photon in an output port is the square of the corresponding amplitude, and, just as for the classical wave, the reflected and transmitted amplitudes have a relative phase.
...
![Figure 1](HOM_figure1_beamsplitter_phase_v2.png)

**Figure 1. One Photon, Two Possibilities.  
A photon entering either port of a 50/50 beamsplitter leaves in a superposition of the two outputs. Both possibilities have amplitude magnitude 1/√2, but reflection from one side introduces a minus sign. This relative phase will become important when two photons arrive together. Image by author.**
...
> **The beamsplitter splits the photon wavefunction in equal parts.**
...
If the photons carried labels, we could distinguish these two outcomes. But identical photons do not carry such labels. 'Both transmitted' and 'both reflected' are therefore two indistinguishable histories leading to exactly the same final state.
...
![Figure_2](HOM_figure2_two_histories_v2.png)
**Figure 2. Two Histories And A Missing Outcome.
...
The deeper reason is the symmetry of identical particles. For bosons, the quantum state must be symmetric under exchange of two particles. For fermions it must be antisymmetric. That minus sign is ultimately responsible for the Pauli exclusion principle for fermions.
...
For two photons on a beamsplitter, we therefore see both destructive and constructive interference between different ways of producing the same outcome. This is different from the familiar interference between classical optical waves. Here it is the *two-photon amplitudes* that interfere.
...
What changes are the amplitudes of the wavefunction for the possible two-photon outcomes. The amplitudes leading to one photon in each output cancel, while the amplitudes associated with two photons occupying the same output mode reinforce.
...
We started with something much more ordinary: light reflecting from a window.
